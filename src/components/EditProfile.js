...
const EditProfile = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (profile) {
      console.log(
        "Fetched image is available:",
        profile.image ? "Available" : "no Image Available"
      );
  ...
  const handleUpdateProfile = () => {
    ...
    if (!isEqual(profile, profileData)) {
      ...
      for (let [key, value] of formData.entries()) {
        console.log(
          `${key}:`,
          value instanceof File ? value.name : value.uri || "Invalid"
        );
      }

      dispatch(
        updateProfileRequest({ id: profileData.id, profileData: formData })
      );
    } else {
